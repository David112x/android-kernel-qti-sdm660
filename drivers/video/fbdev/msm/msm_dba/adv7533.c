...
static int adv7533_read_device_rev(struct adv7533 *pdata)
{
	...
	ret = adv7533_read(pdata, I2C_ADDR_MAIN, ADV7533_REG_CHIP_REVISION,
							&rev, 1);
	pr_debug("ldx adv7533_read_device_rev:0x%x", rev);
	return ret;
}
...
static void adv7533_intr_work(struct work_struct *work)
{
	...
	/* EDID ready for read */
	pr_debug("ldx adv7533_intr_work: int_status:0x%x\n", int_status);
...
static int adv7533_check_hpd(void *client, u32 flags)
{
	...
	pr_debug("ldx adv7533_check_hpd\n");
	if (!pdata) {
		pr_err("%s: invalid platform data\n", __func__);
		return -EINVAL;
	}
	...
	pr_debug("ldx adv7533_read 0x42: 0x%x\n", reg_val);
	...
	struct msm_dba_video_cfg *cfg, u32 flags)
{
	...
	//pr_debug("ldx %s line: %d\n", __func__, __LINE__);
	if (!pdata || !cfg) {
		pr_err("%s: invalid platform data\n", __func__);
		//if(!pdata)
		//    pr_debug("ldx null pdata %s line: %d\n", __func__, __LINE__);

		//if(!cfg)
		//    pr_debug("ldx null cfg %s line: %d\n", __func__, __LINE__);
		return -EINVAL;
	}
...
#ifdef CONFIG_OF
static const struct of_device_id adv7533_dt_ids[] = {
	{ .compatible = "adi,adv7533", },
	{ }
};
MODULE_DEVICE_TABLE(of, adv7533_dt_ids);
#endif
...
static struct i2c_driver adv7533_driver = {
	.driver = {
		.name = "adv7533",
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(adv7533_dt_ids),
	},
	.probe = adv7533_probe,
