	...
	struct device_attribute *attr, char *buf)
{
	...
	/*add 100ms extra delay to wait hpd_state changed*/
	mdelay(100);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rc) {
		pr_notice("%s: kstrtoint failed\n", __func__);
		return -EINVAL;
	}

	pr_notice("%s: set value: %d hpd state: %d\n", __func__,
					hpd, udata->hpd_state);
	if (!hpd) {
...
static void mdss_dba_utils_dba_cb(void *data, enum msm_dba_callback_event event)
{
	...
	pr_notice("event: %d\n", event);
	pr_notice("ldx %s line:%d\n", __func__, __LINE__);

	if (udata->pinfo)
		pluggable = udata->pinfo->is_pluggable;
	
	pr_notice("ldx %s line:%d pluggable:%d\n", __func__, __LINE__, pluggable);
	switch (event) {
...
static const unsigned int hdmi_ext_cable[] = {
	EXTCON_DISP_HDMI,
	EXTCON_NONE,
};

static int mdss_dba_utils_init_switch_dev(struct mdss_dba_utils_data *udata,
	u32 fb_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int rc = -EINVAL, ret;
	...
	udata->sdev_display.supported_cable = hdmi_ext_cable;
					...
					struct mdss_panel_info *pinfo)
{
	...
	pr_notice("%s: current vic code is %d\n", __func__, vic);
...
void mdss_dba_utils_hdcp_enable(void *data, bool enable)
{
	...
	{
	   enable = 0;
	   pr_notice("force to disable hdcp by ldx\n");
	}
