...
#define SITRONIX_TOUCH_RESOLUTION_X 1080 - 1 /* max of X value in display area */
#define SITRONIX_TOUCH_RESOLUTION_Y 1920 - 1 /* max of Y value in display area */
...
static inline void sitronix_ts_pen_down(struct input_dev *input_dev, int id, u16 x, u16 y)
{
    ...
    printk("sdssgl-ts-orig:%s,(%4d,%4d).\n",__func__,x,y);
    //printk("sdssgl-ts-report:%s,(%4d,%4d).\n",__func__,1079-y,x);
	input_report_abs(input_dev, ABS_MT_POSITION_X, 1079 - y);
	input_report_abs(input_dev, ABS_MT_POSITION_Y, x);
    	input_mt_sync_frame(input_dev);
    	input_sync(input_dev);
#if 0
#if 0
	printk("sdssgl %s,the id is %d.\n",__func__,id);
	if((last_x - x) < 10 && (last_y - y) < 10)
	{
		printk("sdssgl filter the pointer(%d,%d).",x,y);
		last_x = x;
		last_y = y;
		filter_flag = 1;
		return ;
...
#else
	/* invert y */
	input_report_abs(input_dev,  ABS_MT_POSITION_X, 1080 - y);
	input_report_abs(input_dev,  ABS_MT_POSITION_Y, x);
	printk("sdssgl %s,(x,y):(%d,%d)",__func__,x,1080 - y);
#endif // SITRONIX_SWAP_XY
...
static inline void sitronix_ts_pen_up(struct input_dev *input_dev, int id)
{
	...
	if(filter_flag == 1)
	{
		printk("sdssgl filter the pointer.\n");
		filter_flag = 0;
		return;
	}
#endif
...
extern void mw_revert_led_status(void);
...
static inline void sitronix_ts_handle_sensor_key(struct input_dev *input_dev, struct sitronix_sensor_key_t *key_array, char *pre_key_status, char cur_key_status, int key_count)
{
	...
	for(i = 0; i < key_count; i++){
		if(cur_key_status & (1 << i)){
			...
			printk("sdssglkey kkk down now key %d down,keycode is 0x%x. \n",cur_key_status,key_array[i].code);
			...
			if(*pre_key_status & (1 << i)){
			    ...
			    printk("sdssglkey kkk down now key %d up,keycode is 0x%x. \n",cur_key_status,key_array[i].code);
                if(KEY_MICMUTE == key_array[i].code)   // Mic Mute
...
static inline void sitronix_ts_handle_aa_key(struct input_dev *input_dev, struct sitronix_AA_key *key_array, char *pre_key_status, char cur_key_status, int key_count)
{
	...
	for(i = 0; i < key_count; i++){
		if(cur_key_status & (1 << i)){
			...
			printk("sdssglkey aa down now key %d,keycode is 0x%x. \n",cur_key_status,key_array[i].code);
			input_report_key(input_dev, key_array[i].code, 1);
			input_sync(input_dev);
		}else{
			if(*pre_key_status & (1 << i)){
...
static int sitronix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("sdssgl %s(%d).\n",__func__,__LINE__);
	sitronix_ts_gpts.reset_pin = of_get_named_gpio(np,"st,reset-gpio",0);
	if ((!gpio_is_valid(sitronix_ts_gpts.reset_pin)))
	{
		ret = -EINVAL;
		dev_err(&client->dev,"%s(%d),reset_pin is invalid,ret is %d.\n",__func__,__LINE__,ret);
		return ret;
	}else{
		error = gpio_request(sitronix_ts_gpts.reset_pin, "touch_reset_gpio");
		if (error) {
				dev_err(&client->dev, "%s: unable to request touch reset gpio [%d]\n",
								__func__, sitronix_ts_gpts.reset_pin);
				//      goto err_en_gpio;
				//      
		}

		gpio_direction_output(sitronix_ts_gpts.reset_pin, 1);
		mdelay(1);
		gpio_direction_output(sitronix_ts_gpts.reset_pin, 0);
		mdelay(5);
	...
	printk("sdssgl %s(%d).\n",__func__,__LINE__);
	client->irq = of_get_named_gpio(np,"st,irq-gpio",0);
	if ((!gpio_is_valid(client->irq)))
	{
		ret = -EINVAL;
		dev_err(&client->dev,"%s(%d),irq_pin is invalid,ret is %d.\n",__func__,__LINE__,ret);
		return ret;
	}else{
...
#ifdef SITRONIX_SUPPORT_MT_SLOT
	__set_bit(BTN_TOUCH, sitronix_ts_gpts.input_dev-> keybit);
	__set_bit(EV_ABS, sitronix_ts_gpts.input_dev->evbit);
	__set_bit(EV_KEY, sitronix_ts_gpts.input_dev->evbit);
	...
	if (client->irq){
		...
		ret = request_irq(client->irq, sitronix_ts_irq_handler,IRQF_TRIGGER_RISING, client->name, &sitronix_ts_gpts);
#else
		ret = request_irq(client->irq, sitronix_ts_irq_handler,IRQF_TRIGGER_RISING , client->name, &sitronix_ts_gpts);
...
#ifdef CONFIG_OF
static const struct of_device_id sitronix_ts_dt_ids[] = {
...
static struct i2c_driver sitronix_ts_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(sitronix_ts_dt_ids),
	},
#ifdef SITRONIX_MULTI_SLAVE_ADDR
