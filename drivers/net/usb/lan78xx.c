			    ...
			    u32 length, u8 *data)
{
	...
	if (ret == 0) {
		if (sig == OTP_INDICATOR_1)
			//offset = offset;
			netdev_warn(dev->net, "lan78xx_dataport sig == OTP_INDICATOR_1");
...
//extern void ksz9563_checkid(void);
extern void i2c_ksz9563_init(void);
static struct phy_device *lan7801_phy_init(struct lan78xx_net *dev)
{
	...
	if (!phydev) {
		...
		ret = lan78xx_write_reg(dev, MAC_RGMII_ID,
					MAC_RGMII_ID_TXC_DELAY_EN_|MAC_RGMII_ID_RXC_DELAY_EN_);
		ret = lan78xx_write_reg(dev, RGMII_TX_BYP_DLL, 0x3D00);
		...
		//ret = lan78xx_read_reg(dev, MAC_CR, &buf);
		//pr_notice("ldx dump reg of MAC_CR: 0x%x\n", buf);
		
		//ret = lan78xx_read_reg(dev, MAC_RGMII_ID, &buf);
		//pr_notice("ldx dump reg of MAC_RGMII_ID: 0x%x\n", buf);
...
static int lan78xx_reset(struct lan78xx_net *dev)
{
	...
	if (dev->chipid == ID_REV_CHIP_ID_7801_)
	{
		...
		//buf &= ~MAC_CR_AUTO_SPEED_;
		//buf |= MAC_CR_AUTO_DUPLEX_ | MAC_CR_AUTO_SPEED_;
		//ret = lan78xx_write_reg(dev, MAC_CR, buf);
		//ret = lan78xx_read_reg(dev, MAC_CR, &buf);
		//pr_notice("ldx lan78xx_reset dump reg of MAC_CR: 0x%x\n", buf);
	}
	...
	#if 0
	ret = lan78xx_read_reg(dev, RX_ADDRH, &buf);
	pr_notice("ldx dump reg of RX_ADDRH: 0x%x\n", buf);
	buf = 0;
	
	ret = lan78xx_read_reg(dev, RX_ADDRL, &buf);
	pr_notice("ldx dump reg of RX_ADDRL: 0x%x\n", buf);
	buf = 0;
	
	ret = lan78xx_read_reg(dev, HW_CFG, &buf);
	pr_notice("ldx dump reg of HW_CFG: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, USB_CFG0, &buf);
	pr_notice("ldx dump reg of USB_CFG0: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, USB_CFG1, &buf);
	pr_notice("ldx dump reg of USB_CFG1: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, USB2_TEST, &buf);
	pr_notice("ldx dump reg of USB2_TEST: 0x%x\n", buf);
	buf = 0;
	...
	ret = lan78xx_read_reg(dev, GPIO_CFG0, &buf);
	pr_notice("ldx dump reg of GPIO_CFG0: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, GPIO_CFG1, &buf);
	pr_notice("ldx dump reg of GPIO_CFG1: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, GPIO_WAKE, &buf);
	pr_notice("ldx dump reg of GPIO_WAKE: 0x%x\n", buf);
	buf = 0;

	ret = lan78xx_read_reg(dev, PIPE_CTL, &buf);
	pr_notice("ldx dump reg of PIPE_CTL: 0x%x\n", buf);
	buf = 0;
	
	
	ret = lan78xx_write_reg(dev, RX_ADDRH, 0x01);
	
	ret = lan78xx_write_reg(dev, RX_ADDRL, 0x780f8000);
	
	ret = lan78xx_write_reg(dev, HW_CFG, 0x3f00010);
	
	ret = lan78xx_write_reg(dev, USB_CFG0, 0x60768164);
	
	ret = lan78xx_write_reg(dev, USB2_TEST, 0x80000000);
	
	ret = lan78xx_write_reg(dev, USB_CFG1, 0x1f00);
	
	ret = lan78xx_write_reg(dev, MAC_CR, 0x380c);
	
	ret = lan78xx_write_reg(dev, GPIO_CFG0, 0xf000);
	
	ret = lan78xx_write_reg(dev, GPIO_CFG1, 0xff000000);
	
	ret = lan78xx_write_reg(dev, GPIO_WAKE, 0x0);
	
	ret = lan78xx_write_reg(dev, PIPE_CTL, 0x0);
	
	pr_notice("ldx dump out again after write\n");
